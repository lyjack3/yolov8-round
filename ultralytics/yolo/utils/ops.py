...
def circle_iou(pred, target):
    ...
    dist = torch.norm(pred_center - target_center, dim=1)
    iou = (pred_radius + target_radius - dist).clamp(min=0) / (pred_radius + target_radius + dist)
    return iou
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if labels and len(labels[xi]):
            ...
            v[:, :3] = lb[:, 1:4]  # circle (cx, cy, r)
            v[range(len(lb)), lb[:, 0].long() + 3] = 1.0  # cls
            x = torch.cat((x, v), 0)
        ...
        # Detections matrix nx6 (cx, cy, r, conf, cls)
        circle, cls, mask = x.split((3, nc, nm), 1)
        if multi_label:
            i, j = (cls > conf_thres).nonzero(as_tuple=False).T
            x = torch.cat((circle[i], x[i, 3 + j, None], j[:, None].float(), mask[i]), 1)
        else:  # best class only
            conf, j = cls.max(1, keepdim=True)
            x = torch.cat((circle, conf, j.float(), mask), 1)[conf.view(-1) > conf_thres]

        # Filter by class
        if classes is not None:
            x = x[(x[:, 4:5] == torch.tensor(classes, device=x.device)).any(1)]

        # Check shape
        n = x.shape[0]  # number of boxes
        if not n:  # no boxes
            continue
        x = x[x[:, 3].argsort(descending=True)[:max_nms]]  # sort by confidence and remove excess boxes

        # Batched NMS
        c = x[:, 4:5] * (0 if agnostic else max_wh)  # classes
        circles, scores = x[:, :3] + c, x[:, 3]  # circles (offset by class), scores
        i = circle_iou(circles, scores)  # NMS
        i = i[:max_det]  # limit detections
        if merge and (1 < n < 3E3):  # Merge NMS (boxes merged using weighted mean)
            # Update circles as circles(i,3) = weights(i,n) * circles(n,3)
            iou = circle_iou(circles[i], circles) > iou_thres  # iou matrix
            weights = iou * scores[None]  # circle weights
            x[i, :3] = torch.mm(weights, x[:, :3]).float() / weights.sum(1, keepdim=True)  # merged circles
            if redundant:
...
def xywh2cxycr(x):
    """Convert [x, y, w, h] to [cx, cy, r]."""
    y = torch.zeros_like(x)
    y[:, 0] = x[:, 0]  # cx
    y[:, 1] = x[:, 1]  # cy
    y[:, 2] = torch.sqrt(x[:, 2] ** 2 + x[:, 3] ** 2) / 2  # r
    return y


def dist2cxycr(dist, anchor_points):
    """Convert distances to [cx, cy, r] format."""
    cx = (anchor_points[:, 0] + dist[:, 0] + dist[:, 2]) / 2
    cy = (anchor_points[:, 1] + dist[:, 1] + dist[:, 3]) / 2
    r = torch.sqrt((dist[:, 2] - dist[:, 0]) ** 2 + (dist[:, 3] - dist[:, 1]) ** 2) / 2
