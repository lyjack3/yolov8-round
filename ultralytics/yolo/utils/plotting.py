...
class Annotator:
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            self.draw.ellipse([box[0], box[1], box[2], box[3]], outline=color, width=self.lw)  # circle
            if label:
                if self.pil_9_2_0_check:
                    _, _, w, h = self.font.getbbox(label)  # text width, height (New)
                else:
                    w, h = self.font.getsize(label)  # text width, height (Old, deprecated in 9.2.0)
                outside = box[1] - h >= 0  # label fits outside circle
                self.draw.rectangle(
            ...
            cv2.circle(self.im, center, radius, color, thickness=self.lw, lineType=cv2.LINE_AA)
            if label:
                tf = max(self.lw - 1, 1)  # font thickness
                w, h = cv2.getTextSize(label, 0, fontScale=self.lw / 3, thickness=tf)[0]  # text width, height
                outside = center[1] - radius - h >= 3
                text_pos = (center[0] - w // 2, center[1] - radius - h - 3 if outside else center[1] + radius + h + 3)
                cv2.rectangle(self.im, text_pos, (text_pos[0] + w, text_pos[1] + h), color, -1, cv2.LINE_AA)  # filled
                cv2.putText(self.im,
                            label, (text_pos[0], text_pos[1] + h if outside else text_pos[1]),
                            0,
                            self.lw / 3,
                            txt_color,
                            thickness=tf,
